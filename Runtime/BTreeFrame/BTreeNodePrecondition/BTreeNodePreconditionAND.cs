...
namespace BTreeFrame
{
    ...
    {
        public BTreeNodePrecondition[] m_Preconditions;
        public BTreeNodePreconditionAND() { }
        public BTreeNodePreconditionAND(params BTreeNodePrecondition[] param)
        {
            if (param == null)
            {
                UnityEngine.Debug.Log("BTreeNodePreconditionAND is null");
                return;
            }
            if (param.Length == 0)
            {
                UnityEngine.Debug.Log("BTreeNodePreconditionAND's length is 0");
                return;
            }
