...
namespace BTree.Editor
{
    class BTreeEditorNodeFactory
    {
        ...
        public static BTreeGraphDesigner BtreeEditorConfig_TO_BTreeGraphDesigner(BTreeEditorConfig _config)
        {
            BTreeGraphDesigner _graphDesigner = new BTreeGraphDesigner();
            _graphDesigner.m_RootNode = BTreeEditorNodeConfig_TO_BTreeNodeDesigner(_config.m_EnterTreeNode);
            ...
            {
                _graphDesigner.m_DetachedNodes.Add(BTreeEditorNodeConfig_TO_BTreeNodeDesigner(_config.m_DetachedTreeNode[i]));
            }
            return _graphDesigner;
        }
        private static BTreeNodeDesigner BTreeEditorNodeConfig_TO_BTreeNodeDesigner(BTreeEditorNodeConfig _config, BTreeNodeDesigner _parent = null)
        {
            BTreeEditorNode editorNode = BTreeEditorHelper.FromJson<BTreeEditorNode>(_config.m_RootNodeJson);
            BTreeNodeDesigner nodeDesigner = new BTreeNodeDesigner(editorNode);
            if (_parent != null)
            {
            ...
            {
                childNodeDesigner.Add(BTreeEditorNodeConfig_TO_BTreeNodeDesigner(_config.m_ChildNodes[i], nodeDesigner));
            }
        ...
        public static string  BTreeNode_TO_Lua(BTreeNodeDesigner _dRoot){
            return BTreeEditorGenLua.GenLuaFromBTreeNode(_dRoot);
        }
        #endregion
    }
}
