...
namespace BTree.Editor
{
    public static class BTreeEditorUtility
    {
        ...
        public static Texture2D LoadTexture(string imageName, bool useSkinColor = true, ScriptableObject obj = null)
        {
            ...
            Stream manifestResourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format("BTreeEditor.Resources.{0}{1}", useSkinColor ? (EditorGUIUtility.isProSkin ? "Dark" : "Light") : "", imageName));
            ...
            if (texture2D == null)
            {
                string path = BTreeEditorHelper.EditorResPath_Package + "{SkinColor}";
            ...
            if (texture2D == null)
            {
                string path = BTreeEditorHelper.EditorResPath + "{SkinColor}";
                texture2D = (AssetDatabase.LoadAssetAtPath(path.Replace("{SkinColor}", EditorGUIUtility.isProSkin ? "Dark" : "Light") + imageName, typeof(Texture2D)) as Texture2D);
            }
            if (texture2D != null)
            {
        ...
        {
            Texture2D texture2D = null;
            Stream manifestResourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format("BTreeEditor.Resources.{0}", iconName.Replace("{SkinColor}", EditorGUIUtility.isProSkin ? "Dark" : "Light")));
            if (manifestResourceStream != null)
            {
                texture2D = new Texture2D(0, 0);
                texture2D.LoadImage(BTreeEditorUtility.ReadToEnd(manifestResourceStream));
            ...
            if (texture2D == null)
            {
                string path = BTreeEditorHelper.EditorResPath_Package;
                texture2D = (AssetDatabase.LoadAssetAtPath(path + iconName, typeof(Texture2D)) as Texture2D);
            }
            if (texture2D == null)
            {
                string path = BTreeEditorHelper.EditorResPath;
                texture2D = (AssetDatabase.LoadAssetAtPath(path + iconName, typeof(Texture2D)) as Texture2D);
            }
            if (texture2D != null)
            {
